...
$select = mysqli_query($database,"SELECT * FROM `orbitusers` WHERE `email`='$forget_email'");
...
if($user > 0){
    ...
    // OTP set in databse
    $update = mysqli_query($database, "UPDATE `orbitusers` SET `otp`='$otp' WHERE `email`='$forget_email'");
